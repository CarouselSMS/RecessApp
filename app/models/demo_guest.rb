class DemoGuest < ActiveRecord::Base
  include AASM
  ...
  validates_presence_of :phone, :user_session_id
  ...
  def send_page!(account)
    unless self.phone.blank?
      ...
      messaging.send_page(normalize_phone(self.phone), 0)
      page!
  ...
  def send_confirmation!(account)
    unless self.phone.blank?
      messaging = Messaging.new(account, account)
      messaging.send_confirmation(normalize_phone(self.phone), 0)
    else
      false
    end
  end
